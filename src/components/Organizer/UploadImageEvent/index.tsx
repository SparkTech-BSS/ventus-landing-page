...
import { MAX_IMAGE_ALLOWED_SIZE } from "config";
import styles from "./styles.module.scss";

export function UploadImageEvent() {
  ...
  const changeHandler = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file?.size > MAX_IMAGE_ALLOWED_SIZE) {
      ...
      <div className={`${styles["upload-image-event-row"]}`}>
        ...
        <div className={`${styles["upload-image-event-info"]}`}>
          ...
          <p className={`${styles["upload-image-event-info--text"]}`}>
            A dimensão recomendada é de <span>1280 x 621</span> (mesma
            proporção do formato utilizado nas páginas de evento no Facebook).
            Formato <span>JPEG, GIF ou PNG de no máximo 2MB</span>. Imagens com
